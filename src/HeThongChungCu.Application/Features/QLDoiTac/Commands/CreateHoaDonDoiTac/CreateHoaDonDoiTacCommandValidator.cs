...
public class CreateHoaDonDoiTacCommandValidator : AbstractValidator<CreateHoaDonDoiTacCommand>
{
    public CreateHoaDonDoiTacCommandValidator()
    {
        RuleFor(x => x.HopDongDoiTacId)
            .GreaterThan(0).WithErrorCode("Validation.InvalidId").WithMessage("ID hợp đồng đối tác không hợp lệ.");
