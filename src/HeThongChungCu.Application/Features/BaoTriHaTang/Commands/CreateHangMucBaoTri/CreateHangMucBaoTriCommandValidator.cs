...
public class CreateHangMucBaoTriCommandValidator : AbstractValidator<CreateHangMucBaoTriCommand>
{
    public CreateHangMucBaoTriCommandValidator()
    {
        RuleFor(x => x.MaHangMuc)
            .NotEmpty().WithMessage("Mã hạng mục bảo trì không được để trống.")
            .MaximumLength(50).WithMessage("Mã hạng mục bảo trì không được vượt quá 50 ký tự.");
