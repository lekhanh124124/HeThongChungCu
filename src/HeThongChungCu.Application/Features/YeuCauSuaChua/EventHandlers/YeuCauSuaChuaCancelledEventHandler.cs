...
public class YeuCauSuaChuaCancelledEventHandler : INotificationHandler<YeuCauSuaChuaCancelledEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaCancelledEventHandler> _logger;

    public YeuCauSuaChuaCancelledEventHandler(
        ...
        ILogger<YeuCauSuaChuaCancelledEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaCancelledEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaCancelledEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Yêu cầu sửa chữa đã bị hủy";
        string content = $"Yêu cầu sửa chữa {yeuCau.NoiDung} đã bị hủy. Lý do: {yeuCau.LyDoHuy}";
