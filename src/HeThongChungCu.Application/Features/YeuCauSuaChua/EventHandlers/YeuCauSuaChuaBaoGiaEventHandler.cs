...
public class YeuCauSuaChuaBaoGiaEventHandler : INotificationHandler<YeuCauSuaChuaBaoGiaEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaBaoGiaEventHandler> _logger;

    public YeuCauSuaChuaBaoGiaEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauSuaChuaQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauSuaChuaBaoGiaEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaBaoGiaEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaBaoGiaEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Thông tin báo giá sửa chữa";
        string content = yeuCau.IsMienPhi == true 
            ? $"Yêu cầu {yeuCau.NoiDung} được xác định là hạng mục miễn phí." 
            : $"Yêu cầu {yeuCau.NoiDung} có báo giá dự kiến: {yeuCau.ChiPhiDuKien?.ToString("N0")} VNĐ.";
