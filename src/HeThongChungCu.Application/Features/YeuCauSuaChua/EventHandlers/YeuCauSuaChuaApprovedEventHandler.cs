...
public class YeuCauSuaChuaApprovedEventHandler : INotificationHandler<YeuCauSuaChuaApprovedEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaApprovedEventHandler> _logger;

    public YeuCauSuaChuaApprovedEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauSuaChuaQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauSuaChuaApprovedEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaApprovedEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaApprovedEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Yêu cầu sửa chữa đã được duyệt";
        string content = $"BQL đã phê duyệt yêu cầu {yeuCau.NoiDung} và đang thực hiện điều phối xử lý.";
