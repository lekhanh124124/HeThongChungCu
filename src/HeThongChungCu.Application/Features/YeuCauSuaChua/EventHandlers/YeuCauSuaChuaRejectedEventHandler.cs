...
public class YeuCauSuaChuaRejectedEventHandler : INotificationHandler<YeuCauSuaChuaRejectedEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaRejectedEventHandler> _logger;

    public YeuCauSuaChuaRejectedEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauSuaChuaQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauSuaChuaRejectedEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaRejectedEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaRejectedEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Yêu cầu sửa chữa bị từ chối";
        string content = $"Yêu cầu sửa chữa {yeuCau.NoiDung} đã bị từ chối. Lý do: {yeuCau.LyDo}";
