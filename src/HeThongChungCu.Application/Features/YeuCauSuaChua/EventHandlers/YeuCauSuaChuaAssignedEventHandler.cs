...
public class YeuCauSuaChuaAssignedEventHandler : INotificationHandler<YeuCauSuaChuaAssignedEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaAssignedEventHandler> _logger;

    public YeuCauSuaChuaAssignedEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauSuaChuaQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauSuaChuaAssignedEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaAssignedEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaAssignedEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Yêu cầu sửa chữa đã được điều phối";
        string content = $"Đã phân công nhân sự xử lý yêu cầu {yeuCau.NoiDung} của bạn.";
