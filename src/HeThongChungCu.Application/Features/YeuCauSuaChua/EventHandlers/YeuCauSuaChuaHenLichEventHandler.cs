...
public class YeuCauSuaChuaHenLichEventHandler : INotificationHandler<YeuCauSuaChuaHenLichEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaHenLichEventHandler> _logger;

    public YeuCauSuaChuaHenLichEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauSuaChuaQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauSuaChuaHenLichEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaHenLichEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaHenLichEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Lịch hẹn sửa chữa";
        string content = $"BQL hẹn lịch xử lý yêu cầu {yeuCau.NoiDung} vào lúc: {yeuCau.HenTu:HH:mm dd/MM} - {yeuCau.HenDen:HH:mm dd/MM}. Vui lòng có mặt tại nhà.";
