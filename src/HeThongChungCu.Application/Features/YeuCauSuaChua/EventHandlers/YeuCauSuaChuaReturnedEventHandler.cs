...
public class YeuCauSuaChuaReturnedEventHandler : INotificationHandler<YeuCauSuaChuaReturnedEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaReturnedEventHandler> _logger;

    public YeuCauSuaChuaReturnedEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauSuaChuaQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauSuaChuaReturnedEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaReturnedEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaReturnedEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Yêu cầu sửa chữa bị trả lại";
        string content = $"Yêu cầu sửa chữa {yeuCau.NoiDung} cần bổ sung thông tin. Lý do: {yeuCau.LyDo}";
