...
public class YeuCauSuaChuaCreatedEventHandler : INotificationHandler<YeuCauSuaChuaCreatedEvent>
{
    ...
    private readonly ILogger<YeuCauSuaChuaCreatedEventHandler> _logger;

    public YeuCauSuaChuaCreatedEventHandler(
        ...
        ILogger<YeuCauSuaChuaCreatedEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauSuaChuaCreatedEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauSuaChua;
        _logger.LogInformation("Handling YeuCauSuaChuaCreatedEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Yêu cầu sửa chữa mới";
        string content = $"Căn hộ {yeuCau.CanHoId} đã gửi yêu cầu sửa chữa mới: {yeuCau.NoiDung}";
