...
public class YeuCauThiCongHoanTatThiCongEventHandler : INotificationHandler<YeuCauThiCongHoanTatThiCongEvent>
{
    ...
    private readonly ILogger<YeuCauThiCongHoanTatThiCongEventHandler> _logger;

    public YeuCauThiCongHoanTatThiCongEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauThiCongQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauThiCongHoanTatThiCongEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauThiCongHoanTatThiCongEvent notification, CancellationToken cancellationToken)
    {
        var yeuCau = notification.YeuCauThiCong;
        _logger.LogInformation("Handling YeuCauThiCongHoanTatThiCongEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Nghiệm thu hoàn tất thi công";
        string content = $"Việc thi công hạng mục {yeuCau.HangMucThiCong} đã được nghiệm thu hoàn tất. Thủ tục hoàn cọc sẽ được xử lý sớm.";
