...
public class YeuCauThiCongHoanCocEventHandler : INotificationHandler<YeuCauThiCongHoanCocEvent>
{
    ...
    private readonly ILogger<YeuCauThiCongHoanCocEventHandler> _logger;

    public YeuCauThiCongHoanCocEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauThiCongQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauThiCongHoanCocEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauThiCongHoanCocEvent notification, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Handling YeuCauThiCongHoanCocEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Hoàn tiền ký quỹ thi công";
        string content = $"Tiền cọc cho yêu cầu {yeuCau.HangMucThiCong} đã được hoàn trả.";
        if (yeuCau.TienKhauTru > 0)
        {
            content += $" (Khấu trừ: {yeuCau.TienKhauTru?.ToString("N0")} VNĐ. Lý do: {yeuCau.LyDoKhauTru})";
        }
