...
public class YeuCauThiCongDaCapPhepEventHandler : INotificationHandler<YeuCauThiCongDaCapPhepEvent>
{
    ...
    private readonly ILogger<YeuCauThiCongDaCapPhepEventHandler> _logger;

    public YeuCauThiCongDaCapPhepEventHandler(
        IThongBaoCommandRepository thongBaoRepository,
        IYeuCauThiCongQueryRepository yeuCauQueryRepository,
        IUnitOfWork unitOfWork,
        INotificationService notificationService,
        IDateTimeProvider dateTimeProvider,
        ILogger<YeuCauThiCongDaCapPhepEventHandler> logger)
    {
    ...
    public async Task Handle(YeuCauThiCongDaCapPhepEvent notification, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Handling YeuCauThiCongDaCapPhepEvent for Request ID: {Id}", yeuCau.Id);

        string title = "Đã cấp phép thi công";
        string content = $"Yêu cầu {yeuCau.HangMucThiCong} đã được cấp phép. Bạn có thể bắt đầu thi công từ ngày {yeuCau.DuKienBatDau:dd/MM/yyyy}.";
