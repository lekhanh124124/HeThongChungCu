...
public class HoanCocCommandHandler : ICommandHandler<HoanCocCommand, YeuCauThiCongResponse>
{
    ...
    public async Task<Result<YeuCauThiCongResponse>> Handle(HoanCocCommand command, CancellationToken cancellationToken)
    {
        var yctc = await _yctcCommandRepository.GetByIdAsync(command.Id, cancellationToken);
        if (yctc == null)
            return YeuCauThiCongErrors.NotFound;

        var result = yctc.HoanCoc(command.TienKhauTru, command.LyDo);
        if (result.IsFailure)
