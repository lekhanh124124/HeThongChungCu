...
public sealed class GeminiLLMService : ILLMService
{
    ...
    public GeminiLLMService(IConfiguration configuration, ILogger<GeminiLLMService> logger)
    {
        ...
        _modelId = configuration["Gemini:ModelId"] ?? "gemini-2.5-flash-lite";
        // Default to low temperature 0.2 for deterministic RAG results
        _temperature = configuration.GetValue<double>("Gemini:LLMTemperature", 0.2);
        _client = new Client(apiKey: apiKey);

        _logger.LogInformation("GeminiLLMService initialized successfully with ModelId: {ModelId}, Temperature: {Temperature}", _modelId, _temperature);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug("Starting LLM content generation. Prompt Length: {PromptLength}, Context Length: {ContextLength}", prompt.Length, context.Length);
        ...
        for (int attempt = 1; attempt <= MaxRetryAttempts; attempt++)
        {
            try
            {
                ...
                config.SystemInstruction = new Content
                {
                    Parts = new List<Part> 
                    { 
                        new Part { Text = combinedInstruction } 
                    }
                };

                _logger.LogDebug("Sending request to Gemini API (Attempt {Attempt}/{MaxAttempts}). ModelId: {ModelId}", attempt, MaxRetryAttempts, _modelId);
                ...
                {
                    _logger.LogError("Gemini API returned an empty or null response on attempt {Attempt} after {DurationMs}ms.", attempt, stopwatch.ElapsedMilliseconds);
                    throw new InvalidOperationException("Gemini model failed to generate any response content.");
                }
            ...
            {
                stopwatch.Stop();
                _logger.LogWarning("LLM generation request was canceled after {DurationMs}ms.", stopwatch.ElapsedMilliseconds);
                throw;
            }
            catch (Exception ex)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var condensePrompt = $"""
            ...
            Nhiệm vụ của bạn là viết lại câu hỏi mới nhất thành một câu hỏi độc lập, đầy đủ ngữ cảnh,
