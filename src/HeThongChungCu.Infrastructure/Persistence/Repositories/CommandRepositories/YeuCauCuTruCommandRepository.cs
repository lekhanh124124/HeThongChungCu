...
public class YeuCauCuTruCommandRepository : IYeuCauCuTruCommandRepository
{
    ...
    {
        return await _dbContext.YeuCauCuTrus
    ...
    {
        return await _dbContext.YeuCauCuTrus.AnyAsync(expression, cancellationToken);
    }

    public async Task AddAsync(YeuCauCuTru yeuCau, CancellationToken cancellationToken = default)
    {
        await _dbContext.YeuCauCuTrus.AddAsync(yeuCau, cancellationToken);
    }

    public void Update(YeuCauCuTru yeuCau)
    {
        _dbContext.YeuCauCuTrus.Update(yeuCau);
    }

    public void Delete(YeuCauCuTru yeuCau)
    {
        _dbContext.YeuCauCuTrus.Remove(yeuCau);
    }

    public void DeleteRange(IEnumerable<YeuCauCuTru> yeuCaus)
    {
        _dbContext.YeuCauCuTrus.RemoveRange(yeuCaus);
    }
}
